...
class ListView(BaseView):
    ...
    def get(self):
        try:
            ...
            logger.warning(f' read action failed with errors: {e}')
            return {'message': str(e)}, 400
        return videos

@videos.get("/tutorials")
@jwt_required()
@marshal_with(VideosSchema(many=True))
def get_list():
    try:
...
@videos.put("/tutorials/<int:tutorial_id>")
@jwt_required()
@use_kwargs(VideosSchema)
@marshal_with(VideosSchema)
...
@videos.delete("/tutorials/<int:tutorial_id>")
@jwt_required()
@marshal_with(VideosSchema)
